...
1. Make sure the font fira code is installed (Debian based distros: sudo apt install font-firacode, Fedora: fira-code-fonts)
2. Run the command: M-x all-the-icons-install-fonts
(3. See Emacs-Client) should have been done by chezmoi, so just make the client the standard application to open text-files
(4. See IDE should be necessary only when I really use it)
5. See Spell-Checking: jinx.el (⟹ On Debian or Ubuntu, install the packages libenchant-2-dev and pkg-config. On Fedora or RHEL, install the package enchant2-devel. On Mac, install enchant2 and pkgconfig.)
...
7. Install aspell dictionaries for languages of choice (Ubuntu: =sudo apt install aspell-de=), on fedora =sudo dnf install aspell-de= and =sudo dnf install aspell=
